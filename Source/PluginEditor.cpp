                                  ...
                                  juce::Slider& slider)
{
  ...
  // Differing colors if bypass buttons are enabled or not
  auto enabled = slider.isEnabled();

  // Color of knobs
  g.setColour(enabled ? pastelRed : Colours::lightgrey); 
  g.fillEllipse(bounds);

  // Color of knob outline
  g.setColour(enabled ? Colours::lightgrey : Colours::darkgrey); 
                                   ...
                                   bool shouldDrawButtonAsDown)
{
  ...
  if( auto* pb = dynamic_cast<PowerButton*>(&toggleButton) )
  {
    ...
    /* // Debugging for seeing power button bounds
    ...
    // Button Colors
    auto color = toggleButton.getToggleState() ? Colours::dimgrey : pastelRed;
...
void RotarySliderWithLabels::paint(juce::Graphics &g)
{
  ...
  // Debugging rectangles for boundries
  // g.setColour(Colours::red);
...
juce::String RotarySliderWithLabels::getDisplayString() const
{
  ...
  // Truncate values above 1000Hz to kHz
  juce::String str;
  bool addK = false;
  ...
  if( auto* floatParam = dynamic_cast<juce::AudioParameterFloat*>(param) )
  {
    ...
    if( val > 999.f )
    {
      val /= 1000.f;  // 10001 / 1000 = 1.001 but we want 1.00
      addK = true;
    }
...
{
  // If analysis is turned off, don't do this stuff
...
void ResponseCurveComponent::paint (juce::Graphics& g)
{
    ...
    // If analyzer is enabled, grab paths and draw them
    if( shouldShowFFTAnalysis )
    {
...
{
  // Create a new background image then create graphics
  // context which will draw into this background image.
  // From there do the usual paint stuff except we are 
  // drawing in the background image directly using the
  // graphics context we created
  using namespace juce;
